    ...
    for batch_i in range(batch_size):
        ...
        for i in range(typo_count[batch_i]):
            ...
            if typo_type[i] == 1:
                ...
                #ok_text.insert(error_index[i], '#')
                #label.insert(error_index[i], 0)
                ok_sample.insert(typo_index[typo_i], alphabet.index('#'))
                #bad_sample.insert(error_index[i], alphabet.index(chr(error_char[i])))
                ...
                #item['ok_sample_one_hot'][batch_i] = torch.cat((item['ok_sample_one_hot'][batch_i][:typo_index[typo_i]], extra_char_one_hot, item['ok_sample_one_hot'][i//4][error_index[i]:-1]), 0)
                ...
                #item['label'][i//4] = torch.tensor(label)
                item['ok_sample'][batch_i] = torch.tensor(ok_sample)
                #item['bad_sample'][i//4] = torch.tensor(bad_sample)
